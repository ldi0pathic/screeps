// Build: 2026-08-03 21:58:41 +02:00
"use strict";
...
function spawn(spawn13, profil, newName, memory) {
  if (spawn13.spawnCreep(profil, newName, { dryRun: true }) === 0) {
    spawn13.spawnCreep(profil, newName, { memory });
    console.log("[" + spawn13.room.name + "|" + memory.workroom + "] spawn " + newName + " cost: " + calcProfil(profil));
    return true;
  }
  return false;
}
...
function _getPriority(structureType) {
...
function spawn2(spawn13, workroom) {
  ...
  if (!bot.room[workroom].sendBuilder || maxbuilder < 1)
    return false;
  if (spawn13.room.name != workroom && !Memory.rooms[workroom].claimed && !bot.room[workroom].claim)
    return false;
  ...
  if (sites == 0 || Math.max(sites / 5, 1) <= count)
    return false;
  return spawn(spawn13, _getProfil(spawn13), role + "_" + Game.time, { role, workroom, home: spawn13.room.name });
}

// src/roles/claimer.ts
var claimer_exports = {};
__export(claimer_exports, {
  _getProfil: () => _getProfil2,
  doJob: () => doJob2,
  sayJob: () => sayJob2,
  spawn: () => spawn3
});
...
function _getProfil2() {
  return [CLAIM, CLAIM, MOVE, MOVE];
}
function spawn3(spawn13, workroom) {
  ...
  var room = Game.rooms[workroom];
  if (room && room.controller && room.controller.sign && (room.controller.sign.username == spawn13.owner.username || room.controller.sign.username == "Screeps") && room.controller.reservation && room.controller.reservation.ticksToEnd > 3e3)
    return false;
  if (1 <= count)
    return false;
  return spawn(spawn13, _getProfil2(), role2 + "_" + Game.time, { role: role2, workroom, home: spawn13.room.name });
}

// src/roles/debitor.ts
var debitor_exports = {};
__export(debitor_exports, {
  _spawn: () => _spawn,
  doJob: () => doJob3,
  getProfil: () => getProfil,
  sayJob: () => sayJob3,
...
function getProfil(spawn13, workroom, mineraltype, containerId) {
  if (mineraltype == RESOURCE_ENERGY) {
    if (spawn13.room.name != workroom) {
      ...
      if (!carry && distances) {
        ...
        var max = Math.min(25, parseInt(spawn13.room.energyCapacityAvailable / 100));
        if (max >= carry) {
    ...
    if (containerId == "" || spawn13.room.name != workroom) {
      var max = Math.min(Math.max(parseInt(spawn13.room.energyCapacityAvailable / 100), 1), 20);
...
function spawn4(spawn13, workroom) {
  if (bot.room[workroom].transferEnergie && spawn13.room.name != workroom || spawn13.room.name != workroom && !Memory.rooms[workroom].claimed)
    return false;
  if (bot.room[workroom].sendDebitor && bot.room[workroom].sendMiner && (!Memory.rooms[workroom].hasLinks || !bot.room[workroom].useLinks)) {
    for (var id in bot.room[workroom].energySources) {
      if (!Game.getObjectById(bot.room[workroom].energySources[id]))
        continue;
      if (_spawn(spawn13, workroom, bot.room[workroom].energySources[id], RESOURCE_ENERGY))
        return true;
    }
  } else if (bot.room[workroom].sendFreeDebitor) {
    if (_spawn(spawn13, workroom, "", RESOURCE_ENERGY))
      return true;
  }
  return false;
}
function _spawn(spawn13, workroom, source, mineraltype) {
  ...
  var profil = getProfil(spawn13, workroom, mineraltype, containerId);
  bot.logWorkroom(workroom, "4");
  if (!spawn(spawn13, profil, role3 + "_" + Game.time, { role: role3, harvest: true, workroom, home: spawn13.room.name, mineral: mineraltype, container: containerId, notfall: false })) {
    if (_.filter(Game.creeps, (creep) => creep.memory.role == role3 && creep.memory.workroom == workroom).length == 0 && spawn13.room.name == workroom) {
      console.log("[" + spawn13.room.name + "|" + workroom + "]Notfallspawn Debitor");
      var min = Math.min(Math.max(parseInt(spawn13.room.energyAvailable / 100), 1), 16);
      profil = Array(min).fill(CARRY).concat(Array(min).fill(MOVE));
      mineraltype = RESOURCE_ENERGY;
      return spawn(spawn13, profil, role3 + "_" + Game.time, { role: role3, harvest: true, workroom, home: spawn13.room.name, mineral: mineraltype, container: "", notfall: true });
    }
    return false;
  }
  return true;
...
var defender_exports = {};
__export(defender_exports, {
  _defend: () => _defend,
  _getProfil: () => _getProfil3,
  doJob: () => doJob4,
  sayJob: () => sayJob4,
  spawn: () => spawn5
});
...
function doJob4(creep) {
  if (goToWorkroom2(creep)) return;
  if (_defend(creep)) return;
}
function _defend(creep) {
  var _a;
  if (creep.room.name != creep.memory.workroom)
    return false;
...
function spawn5(spawn13, workroom) {
  ...
  if (Memory.rooms[workroom].needDefence && 2 <= count || Memory.rooms[workroom].invaderCore && 4 <= count)
    return false;
  if (spawn(spawn13, _getProfil3(spawn13), role4 + "_" + Game.time, { role: role4, workroom, home: spawn13.room.name })) {
...
var extupgrader_exports = {};
__export(extupgrader_exports, {
  _getProfil: () => _getProfil4,
  doJob: () => doJob5,
  sayJob: () => sayJob5,
  spawn: () => spawn6
});
...
function _getProfil4(spawn13, workroom) {
  var numberOfSets = 0;
...
function spawn6(spawn13, workroom) {
  if (spawn13.room.name == workroom)
    return false;
  ...
  if (uppis <= count)
    return false;
  var profil = _getProfil4(spawn13, workroom);
  return spawn(spawn13, profil, role5 + "_" + Game.time, { role: role5, workroom, home: spawn13.room.name, repairs: 0 });
}

// src/roles/miner.ts
var miner_exports = {};
__export(miner_exports, {
  _clearMemory: () => _clearMemory,
  _getProfil: () => _getProfil5,
  _spawn: () => _spawn2,
  doJob: () => doJob6,
  sayJob: () => sayJob6,
  spawn: () => spawn7
...
function _getProfil5(spawn13, workroom) {
...
function spawn7(spawn13, workroom) {
  ...
  for (var id in bot.room[workroom].energySources) {
    if (!Game.getObjectById(bot.room[workroom].energySources[id]))
      continue;
    if (_spawn2(spawn13, workroom, bot.room[workroom].energySources[id], true))
      return true;
  }
  var room = Game.rooms[workroom];
  if (room && room.controller && room.controller.my && room.controller.level >= 6) {
    for (var id in bot.room[workroom].mineralSources) {
      ...
      if (!mineral || mineral.mineralAmount < 1)
        return false;
      if (_spawn2(spawn13, workroom, bot.room[workroom].mineralSources[id], false))
        return true;
    }
  }
  return false;
}
function _spawn2(spawn13, workroom, source, mineEnergy) {
  var time = 300;
  if (workroom == spawn13.room.name) {
    time = 150;
  }
  var count = _.filter(
    Game.creeps,
  ...
  if (!spawn(spawn13, _getProfil5(spawn13, workroom), role6 + "_" + Game.time, { role: role6, workroom, home: spawn13.room.name, source, mineEnergy, notfall: false })) {
    Memory.rooms[spawn13.room.name].aktivPrioSpawn = true;
    Memory.rooms[spawn13.room.name].aktivPrioSpawnCount = (Memory.rooms[spawn13.room.name].aktivPrioSpawnCount || 0) + 1;
    if (Memory.rooms[spawn13.room.name].aktivPrioSpawnCount > 25) {
      if (_.filter(Game.creeps, (creep) => creep.memory.role == role6 && creep.memory.workroom == workroom && creep.memory.source == source).length > 0)
        return false;
      console.log("[" + spawn13.room.name + "|" + workroom + "] Spawn NotfallMiner!!!");
      spawn(spawn13, [WORK, CARRY, MOVE], role6 + "_" + Game.time, { role: role6, workroom, home: spawn13.room.name, source, mineEnergy, notfall: true });
      Memory.rooms[spawn13.room.name].aktivPrioSpawnCount = 0;
      return true;
    }
    return false;
  }
...
var repairer_exports = {};
__export(repairer_exports, {
  _getMinHitRange: () => _getMinHitRange,
  _getPriority: () => _getPriority2,
  _getProfil: () => _getProfil6,
  _repair: () => _repair,
  _repairPrio: () => _repairPrio,
  doJob: () => doJob7,
  sayJob: () => sayJob7,
  spawn: () => spawn8
});
...
function _getPriority2(structureType) {
  return bot.prio.repair[structureType] || 99;
}
function _getMinHitRange(structureType) {
  return bot.prio.hits[structureType] || 0.5;
}
function _repairPrio(creep) {
...
function spawn8(spawn13, workroom) {
  var minRepairer = bot.room[workroom].repairer;
  if (minRepairer < 1)
    return false;
  if (spawn13.room.name != workroom && !Memory.rooms[workroom].claimed)
    return false;
  ...
  if (structuresToRepair.length <= 1)
    return false;
  return spawn(spawn13, _getProfil6(spawn13), role7 + "_" + Game.time, { role: role7, workroom, home: spawn13.room.name, repairs: 0 });
}
...
function getProfil2(spawn13) {
  var max = Math.min(25, parseInt(spawn13.room.energyCapacityAvailable / 100));
  return Array(max).fill(CARRY).concat(Array(max).fill(MOVE));
}
function spawn9(spawn13, workroom) {
  if (!bot.room[workroom].transferEnergie || spawn13.room.name == workroom || !Memory.rooms[workroom].claimed)
    return false;
  if (_spawn3(spawn13, workroom, RESOURCE_ENERGY))
    return true;
  return false;
}
function _spawn3(spawn13, workroom, mineraltype) {
  var count = _.filter(
    Game.creeps,
    (creep) => creep.memory.role == role8 && creep.memory.workroom == workroom && creep.memory.home == spawn13.room.name && //hier wichtig, da mehere spawns infrage kommem
    (creep.ticksToLive > 100 || creep.spawning)
  ).length;
  if (1 <= count)
    return false;
  var storage = Game.rooms[spawn13.room.name].storage;
  if (storage && storage.store[RESOURCE_ENERGY] < 1e4 || !storage)
    return false;
  var profil = getProfil2(spawn13);
  return spawn(spawn13, profil, role8 + "_" + Game.time, { role: role8, harvest: true, workroom, home: spawn13.room.name, mineral: mineraltype });
}

// src/roles/upgrader.ts
var upgrader_exports = {};
__export(upgrader_exports, {
  _getProfil: () => _getProfil7,
  doJob: () => doJob9,
  sayJob: () => sayJob9,
  spawn: () => spawn10
});
...
function _getProfil7(spawn13, workroom) {
  var numberOfSets = 0;
...
function spawn10(spawn13, workroom) {
  var uppis = bot.room[workroom].upgrader;
  if (!uppis || uppis < 1)
    return false;
  if (spawn13.room.name != workroom)
    return false;
  if (spawn13.room.controller.level > 7 && spawn13.room.controller.ticksToDowngrade > 1e5 && spawn13.room.storage && spawn13.room.storage.store.getUsedCapacity(RESOURCE_ENERGY) < 25e4)
    return false;
  var count = _.filter(
    Game.creeps,
  ...
  if (uppis <= count)
    return false;
  var profil = _getProfil7(spawn13, workroom);
  return spawn(spawn13, profil, role9 + "_" + Game.time, { role: role9, workroom, home: spawn13.room.name, repairs: 0, noLink: false });
}

// src/roles/wally.ts
var wally_exports = {};
__export(wally_exports, {
  _getProfil: () => _getProfil8,
  _repair: () => _repair2,
  doJob: () => doJob10,
  sayJob: () => sayJob10,
  spawn: () => spawn11
...
function doJob10(creep) {
  ...
  if (goToWorkroom2(creep)) return;
  if (checkWorkroomPrioSpawn(creep)) return;
  if (_repair2(creep)) return;
  upgradeController(creep);
}
function _repair2(creep) {
  var targetWall;
...
function spawn11(spawn13, workroom) {
  if (spawn13.room.name != workroom && !Memory.rooms[workroom].claimed)
    return false;
  ...
  if (storage && storage.store[RESOURCE_ENERGY] < 5e4 || !storage)
    return false;
  var p = _getProfil8(spawn13);
  return spawn(spawn13, p, role10 + "_" + Game.time, { role: role10, workroom, home: spawn13.room.name });
}

// src/roles/index.ts
var jobs = {
  debitor: debitor_exports,
  transfer: transfer_exports,
  miner: miner_exports,
  claimer: claimer_exports,
  builder: builder_exports,
  repairer: repairer_exports,
  upgrader: upgrader_exports,
  extupgrader: extupgrader_exports,
  defender: defender_exports,
  wally: wally_exports
};
...
function spawn12() {
  for (const spawnName in Game.spawns) {
    const spawn13 = Game.spawns[spawnName];
    if (!spawn13 || spawn13.spawning) continue;
    const emergencyCreeps = Object.values(Game.creeps).filter((creep) => {
      const memory = creep.memory;
      return memory.home === spawn13.room.name && memory.notfall;
    });
    for (const roomName in bot.room) {
      const config = bot.room[roomName];
      if (!config) continue;
      const workroom = config.room;
      if (emergencyCreeps.length > 0 && workroom !== spawn13.room.name) {
        bot.logWorkroom(workroom, `has NotfallCreep! >> ${JSON.stringify(emergencyCreeps)}`);
        continue;
      }
      const transfer = bot.transfer[workroom];
      if ((transfer == null ? void 0 : transfer.source.includes(spawn13.room.name)) && jobs.transfer.spawn(spawn13, workroom)) {
      ...
      if (config.sendDefender && (roomMemory.needDefence || roomMemory.invaderCore)) {
        jobs.defender.spawn(spawn13, workroom);
        bot.logWorkroom(workroom, "Spawn Defender");
        continue;
      }
      if (config.spawnRoom !== spawn13.room.name && config.room !== spawn13.room.name) continue;
      if (roomMemory.invaderCore) continue;
      bot.logWorkroom(workroom, "Spawn JobLoop");
      for (const jobName in jobs) {
        bot.logWorkroom(workroom, `Spawn Job: ${jobName}`);
        if (jobs[jobName].spawn(spawn13, workroom)) break;
      }
      if (spawn13.spawning) break;
    }
  }
}
