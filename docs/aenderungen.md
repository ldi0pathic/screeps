...
| `controller/defence.ts` · `tower` (Reparatur) | **Reparatur: alle Türme reparierten das gleiche Struktur. Fix: jedem Turm wird eine andere Struktur aus der sortierten Liste zugewiesen, sodass die Reparaturlast verteilt wird (i % structureCount).** | `damagedStructures[0]` → `damagedStructures[i % structureCount]` | Türme reparieren nun verschiedene Strukturen, nicht alle dieselbe gleichzeitig. |
