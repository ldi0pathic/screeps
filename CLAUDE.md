...
Vor jedem Build sichert `build-common.ts::backupMainJs()` die vorhandene `tsProd/main.js` nach `tsProd-backup/main-<Stempel der gesicherten Datei>.js` — im Watch-Modus an esbuilds `onStart`, weil `onEnd` zu spät liefe. Es werden die 20 neuesten Sicherungen behalten, ältere löscht der Build. Ein fehlgeschlagenes Backup warnt nur und bricht den Build nicht ab.

`tsProd-backup/` ist **eingecheckt** und liegt bewusst als Schwesterordner neben `tsProd/`, nicht darin: `uploader.ts` lädt jede `.js` aus `tsProd` als eigenes Spielmodul hoch, und der GitHub-Sync des Spiels zeigt auf `tsProd/`. Ein Ordner daneben ist für beide unsichtbar. Nebenwirkung: jeder Build erzeugt dort eine neue und löscht die einundzwanzigste Datei, das ist bei jedem Commit im Diff zu sehen.
...
- **Beschaffungsketten (`src/creep/target.ts`)**: `RememberedTarget` für die `useX`-Memory-Schlüssel, `collectFrom`/`withdrawFrom`/`transferTo` für die Auswertung einer Aktion am Ziel (`ERR_NOT_IN_RANGE` → hinlaufen, `OK` → `fromId` setzen, sonst aufgeben). Nicht wieder als `switch` in die einzelne Kette schreiben. Zwei Regeln, die dort absichtlich so stehen: ein gemerktes, aber verschwundenes Ziel löst **keine** Ersatzsuche aus, und Schwellenvergleiche (`store[type] > min`) bleiben **positiv** formuliert — bei einer fehlenden Ressource ist der Wert `undefined`, und dann sind `>` und negiertes `<=` beide falsch.
- **Containerliste (`src/creep/containers.ts`)**: `ContainerList` kapselt `Memory.rooms[<raum>].container` samt Neuerhebung und Nächster-Suche. Zwei absichtliche Asymmetrien zwischen Holen und Abliefern stehen dort dokumentiert (`hasList` gegen `hasEntries`, `forgetListOnStaleId`) — nicht „vereinheitlichen", das wäre eine Verhaltensänderung.
- **Rumpfprofile (`src/creep/bodies.ts`)**: die Rümpfe aller Rollen an einer Stelle, die Arithmetik in der Klasse `BodyProfile` (`src/creep/body.ts`). Rollen wählen nur noch aus (RCL, Heimatraum) und rechnen nicht selbst. Wer eine Zahl ändert, ändert sie dort — und `tests/creep-bodies.test.ts` führt die alten Formeln als Referenz mit, ein geänderter Rumpf fällt also sofort auf. Ein Rückfallprofil ist **Pflicht**: ein leeres Body-Array lässt `spawnCreep` immer fehlschlagen, dieser Fehler ist im Repo schon dreimal aufgetreten.
...
- **Statische Konfiguration = `global.*`**, gesetzt von `prod/config.js` (JS-Bot) und `tsBot/src/config.ts` (TS-Bot). `global.room` ist die maßgebliche Liste der verwalteten Räume und steuert Spawning, Rollenverteilung und Limits (`spawnRoom`, `sendMiner`/`sendDebitor`/…, Quellen-IDs, `maxbuilder`, `maxwallRepairer`, `claim`, `saveRoads`). Dazu `global.prio.build/repair/hits` (kleinerer Wert = höhere Priorität; gemeinsam genutzt von Builder, Repairer und Towern), `global.transfer` sowie die Helfer `global.log`/`global.logWorkroom`. **Faustregel für neue Schlüssel: Absicht gehört in die Config, Tatsachen über die Welt nicht** — Link-IDs und `useLinks` standen früher hier und werden inzwischen aus der Lage bzw. dem RCL abgeleitet (`controller/link-list.ts`). Räume oder Limits für den TS-Bot ändern heißt: `src/config.ts` editieren. Das Modul wirkt nur über Seiteneffekte und wird deshalb als **erster** Import in `main.ts` geladen — diese Reihenfolge nicht verändern.
...
Rollen sind in `roles/index.ts` registriert (`debitor`, `transfer`, `miner`, `claimer`, `builder`, `repairer`, `upgrader`, `extupgrader`, `defender`, `wally`); jede exportiert `doJob(creep)` und `spawn(spawn, workroom)`. Die Property-Reihenfolge dieser Tabelle *ist* die Spawn-Priorität, die `controller/spawn.ts` verwendet — nicht umsortieren, und die Schlüssel nicht umbenennen (sie stehen im laufenden Spiel im Creep-Memory). Gemeinsames Creep-Verhalten liegt in `creep/base.ts` (+ `goto.ts`, `transport.ts`): das Flag `memory.harvest` ist der Zustandsautomat Beschaffen/Abliefern, umgeschaltet von `checkHarvest()`; `memory.fromId` verhindert das Zurückliefern in die eben genutzte Quelle; `moveByMemory` cacht einen serialisierten Pfad in `memory.path`/`memory.pathTarget` — deshalb wird der Cache bei jedem Zielwechsel verworfen. Die vier Schlüssel (`path`, `pathTarget`, `lastPos`, `dontMove`) liegen hinter der Klasse `PathMemory` (`src/creep/path-memory.ts`), die **zwei** Löschregeln unterscheidet: `forgetPath()` nur den Weg, `clear()` zusätzlich die Stauerkennung. Nicht wieder per `delete` an den Schlüsseln arbeiten — der Unterschied zwischen beiden Regeln ist genau der Fehler, den man dabei macht.
